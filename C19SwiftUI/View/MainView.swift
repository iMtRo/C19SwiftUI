...
struct MainView: View {
    ...
    @ObservedObject var data = getData(country: "Uzbekistan")
    @State var country: String = "Uzbekistan"
            ...
            if self.data.data != nil {
                ZStack{
                    VStack{
                        ...
                        ZStack{
                            HStack {
                                TextField("Enter Country Here", text: $country).foregroundColor(.white)
                                    .padding(10)
                                    .font(Font.system(size: 15, weight: .medium, design: .serif))
                                    .frame(width: 380, height: 50)
                                    .overlay(RoundedRectangle(cornerRadius: 8).stroke(Color.white, lineWidth: 1))
                                Button(action:{ self.data.updateData(country: self.country) }){
